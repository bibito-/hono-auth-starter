...
**`.claude/specs/`・`.claude/docs/` への書き込み、INDEX.md の更新、doc の改訂を行う前に必ず全文を読むこと。**

常時適用される最低限のルール:

- ライフサイクルは `.claude/specs/`（実装前の仕様書）→ 実装 → `.claude/docs/`（永続アーカイブ）
- `.claude/docs/` を参照するときは同フォルダで最も連番が高いファイルを読む
