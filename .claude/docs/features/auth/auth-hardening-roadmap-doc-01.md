...
## 現状のセキュリティスコア（ai-todo における評価時点のスコア）

| 層 | スコア | 主な問題 |
|---|---|---|
| クライアント → Supabase（todos） | 8/10 | RLS 機能している（全 CRUD ポリシー済み） |
| クライアント → Supabase（profiles） | 7/10 | manager_update_staff バグ修正済み（2026-06-21）|
| クライアント → Hono（/api/todos/analyze） | 1/10 | 認証ゼロ・userId をボディから信頼 |
| TodoAgent → Supabase | 5/10 | service_role で RLS バイパス。userId が Hono 未検証由来 |
| クライアント RBAC | 4/10 | role はクライアントのみ。API 側に RBAC なし |
