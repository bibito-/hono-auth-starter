...
`.github/workflows/stack-kit-pull-check.yml` もマニフェストに含まれる。ただし CI はこれを適用できない（`GITHUB_TOKEN` は `.github/workflows/` の作成・更新を拒否し、自動PRの push が `remote rejected` になる）。CI は差分があることを `::warning::` で知らせるだけで、実際の取り込みはこのスキルを人間が実行したときに行われる。
