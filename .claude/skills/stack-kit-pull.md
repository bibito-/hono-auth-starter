...
### Step 0: 自プロジェクトが origin/main に追随しているか確認する

base SHA は作業ツリーの `.claude/manifests/stack-kit-base.txt` から読むため、**チェックアウトしているブランチによって値が変わる**。前回の pull より前に切られたフィーチャーブランチ上で実行すると base が1世代古く、すでに取り込み済みの変更が「未取り込み」に見える。

これは黙って誤動作する。Step 7 の doc-push-agent は main へコミットするため、その誤判定のまま進むと main に対して適用済みの変更を再適用したり、base SHA を古い値へ巻き戻すコミットを積む。
...
3行目が**空**であること（HEAD が origin/main を含む）を確認する。空でなければ、このブランチは origin/main より古い。中断してユーザーに報告し、main へ切り替えるか main を取り込んでからやり直す。

> `git merge-base --is-ancestor` の終了コードだけで判定しないこと。ラッパー越しに終了コードが潰れて常に成功に見えることがある。`git log` の出力が空かどうかで判定する。
