...
base SHA は作業ツリーの `.claude/manifests/workflow-kit-base.txt` から読むため、**チェックアウトしているブランチによって値が変わる**。前回の pull より前に切られたフィーチャーブランチ上で実行すると base が1世代古く、すでに取り込み済みの変更が「未取り込み」に見える。
