...
```bash
git -C ../hono-auth-starter fetch origin main -q
git -C ../hono-auth-starter rev-parse HEAD origin/main
```
...
コピー元プロジェクト固有の内容が紛れていないかを、専任エージェント `kit-push-review-agent` に審査させる。agent は文意レベルで「この手順文や構成は Hono+Supabase スタック全般に通用するか、それとも特定プロジェクトの都合が混ざっているか」を判断する（grep では表現できない線引き）。

スタック層では、さらに `docs/rules/` のファイルについて、そこに書かれたコードの構造（定数名・関数名・設定キー）が hono-auth-starter の `src/` に実在するかも確認する。存在しなければ contaminated とする。コピー元プロジェクト固有に進化したコードの説明であり、そのまま流すとドキュメントだけが同期されコードが追随しない状態になるため。

**rules を書き戻してはならない**（コピー元の rules は自分のコードと一致しているため、書き戻すと同一パスのファイルが両リポジトリで別々に正しくなり、pull/push のたびに往復する）。対応は以下のどちらか：
- コード側を hono-auth-starter へ移植して構造を収束させる（推奨。starter はテンプレの正なので新規プロジェクトが良い設計を継げる）
- rules から実装詳細への言及を外し、構造に依存しない記述にする

実例（2026-07-11）: `local-dev-rules.md` が `LOCAL_ORIGINS` と `vars.PROD_VERCEL_ORIGIN` を説明していたが、hono-auth-starter の `src/server/cors.ts` は `ALLOWED_ORIGINS`（本番オリジン直書き）のままだった。ドキュメントだけが同期され、コードが追随していなかった。

```bash
/kit-push-review-agent \
  kit_path="../hono-auth-starter" \
  universality_criterion="Hono+Supabase スタック全般に通用すること。スタック語（hono / supabase / wrangler 等）は当然含んでよく、弾くのはコピー元プロジェクト固有名のみ。docs/rules/ ファイルについては、コードの構造（定数名・関数名・設定キー）が src/ に実在するか確認し、実在しなければ contaminated とする" \
...
```bash
git -C ../hono-auth-starter switch -c kit-push/$(date +%Y%m%d)-<要約>
git -C ../hono-auth-starter add <変更ファイルのみ列挙>
git -C ../hono-auth-starter commit -m "$(cat <<'EOF'
<変更内容の要約（日本語）>
...
```bash
# 修正を加えたあと
git -C ../hono-auth-starter add <修正したファイル>
git -C ../hono-auth-starter commit --amend --no-edit   # トレーラを保ったまま HEAD を差し替える
git -C ../hono-auth-starter push --force-with-lease
```
