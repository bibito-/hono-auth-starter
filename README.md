...
このテンプレートは GitHub Template repository なので、複製時に `src/` やマニフェスト記載ファイルは全部届きます。
唯一同期に必要なのは正のリポジトリのSHA値です。　`.claude/manifests/stack-kit-base.txt`　が無いと `stack-kit-pull-check.yml` が共通祖先を決められず、初回 CI が `noancestor` で必ず固まる。
