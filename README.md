...
### 2. stack-kit の同期ループに結線する（必須）

このテンプレートは GitHub Template repository なので、複製時に `src/` やマニフェスト記載ファイルは全部届く。だが **`.claude/manifests/stack-kit-base.txt`（正のリポジトリが持たないファイル）と Secrets は複製されない**。この2つが無いと `stack-kit-pull-check.yml` が共通祖先を決められず、初回 CI が `noancestor` で必ず固まる。

hono-auth-starter を隣にクローンしたうえで、複製したプロジェクトのルートで実行する:

```
../hono-auth-starter/scripts/stack-kit-scaffold.sh
```

スクリプトはファイル配置とレポートのみを行い、git 操作はしない。実行後、レポートに出る手動作業2件（`STACK_KIT_PAT` の登録・`.claude/settings.json` へのフック登録）を済ませること。

`.claude/` の core 層（スタック非依存のワークフロー基盤）も要る場合は、[claude-workflow-kit](https://github.com/bibito-/claude-workflow-kit) の `scripts/scaffold.sh` を先に実行する。

### 3. Supabase プロジェクトを新規作成する
...
□ `.claude/manifests/stack-kit-base.txt` が存在し、`stack-kit-pull-check` が `noancestor` warning なしで完走する
